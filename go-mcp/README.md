...
[MCP](https://modelcontextprotocol.io) server exposing the WaybackMachine SDK as
two agent tools — `wayback-machine_list` and `wayback-machine_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/wayback-machine-mcp)
make build

# 2. Provide credentials via the environment
export WAYBACK_MACHINE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user wayback-machine \
  -- /absolute/path/to/wayback-machine-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./wayback-machine-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// wayback-machine_list: first page of records
{ "entity": "availability" }
{ "entity": "availability", "query": { } }

// wayback-machine_load: one record by id
{ "entity": "availability", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/wayback-machine-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user wayback-machine \
     -- "$PWD"/dist/*/wayback-machine-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `wayback-machine_list` and `wayback-machine_load` tools now appear
   in new sessions. Ask the agent to *"list availability using wayback-machine"*
   and it calls `wayback-machine_list` with `{"entity":"availability"}`.
...
```sh
export WAYBACK_MACHINE_APIKEY=sk_live_xxx            # API key
export WAYBACK_MACHINE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `wayback-machine_list` tool
...
### Call the `wayback-machine_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `wayback-machine_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `wayback-machine_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `WAYBACK_MACHINE_APIKEY` | API key sent with every request. |
| `WAYBACK_MACHINE_BASE` | Optional override of the API base URL. |
...
```sh
./wayback-machine-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"wayback-machine_load","arguments":{"entity":"availability","query":{"id":1}}}}'
```
