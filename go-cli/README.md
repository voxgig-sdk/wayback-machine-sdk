...
boru-driven command-line client **and** interactive REPL for the WaybackMachine
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/wayback-machine-cli)
make build

# 2. See usage (words, entities, env vars)
./wayback-machine-cli --help

# 3. Provide credentials once, via the environment
export WAYBACK_MACHINE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./wayback-machine-cli load 1 availability            # {id:1} shorthand
./wayback-machine-cli load '{id:1}' availability       # explicit match map

# 5. Override the API base URL for a single call
WAYBACK_MACHINE_BASE=https://api.example.com ./wayback-machine-cli load 1 availability

# 6. No arguments -> interactive REPL
./wayback-machine-cli
wayback-machine> load 1 availability
wayback-machine> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/wayback-machine-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/wayback-machine-cli load 1 availability
   ```
...
```sh
./wayback-machine-cli load 1 availability          # scalar shorthand for {id:1}
./wayback-machine-cli load '{id:1}' availability     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `wayback-machine>`). Each line is
evaluated as its own boru expression:

```text
$ ./wayback-machine-cli
wayback-machine> load 1 availability
wayback-machine> /help
wayback-machine> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `availability`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/wayback-machine-cli`. |
...
(`./wayback-machine-cli <expr>`) and interactively (the REPL), and expressions compose the
