...
class AvailabilityEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set WAYBACK_MACHINE_TEST_AVAILABILITY_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    availability_ref01_data_dt0_loaded = availability_ref01_ent.load(availability_ref01_match_dt0, nil)
...
def availability_basic_setup(extra)
  ...
  entid_env_raw = ENV["WAYBACK_MACHINE_TEST_AVAILABILITY_ENTID"]
