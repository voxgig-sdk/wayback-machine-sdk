...
import (
	...
	omni "github.com/voxgig-sdk/wayback-machine-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.WaybackMachineError); is {
				out := map[string]any{
					"name":    "WaybackMachineError",
...
func omniCtx(arg any, client *sdk.WaybackMachineSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.WaybackMachineSDK); is {
			client = live
