...
func TestAvailabilityEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		availabilityRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.availability")))
...
func availabilityBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("WAYBACK_MACHINE_TEST_AVAILABILITY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["WAYBACK_MACHINE_TEST_AVAILABILITY_ENTID"])
