...
// WaybackMachine SDK retry feature
...
class WaybackMachineRetryFeature extends WaybackMachineBaseFeature
{
    ...
    public function init(WaybackMachineContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (WaybackMachineContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(WaybackMachineContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(WaybackMachineContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
