...
module WaybackMachineUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = WaybackMachineResponse.new({})
    ...
    if fetch_err
      ...
      response = WaybackMachineResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = WaybackMachineResponse.new(fetched)
    else
