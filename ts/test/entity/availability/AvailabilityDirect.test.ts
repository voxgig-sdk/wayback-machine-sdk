...
describe('AvailabilityDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new WaybackMachineSDK({
  ...
  test('direct-load-availability', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-availability', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['WAYBACK_MACHINE_TEST_AVAILABILITY_ENTID']
