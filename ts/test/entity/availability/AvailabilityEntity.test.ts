...
describe('AvailabilityEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'availability.' + op, live)) return
    }
    ...
    const availability_ref01_data_dt0 = await availability_ref01_ent.load(availability_ref01_match_dt0)
