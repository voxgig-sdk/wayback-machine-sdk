...
//     (\`new WaybackMachineSDK(...)\` / \`WaybackMachineSDK.test(...)\` become
//     \`WaybackMachineSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'WaybackMachineSDK'
...
const TEST_SEED = {"entity":{"availability":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new WaybackMachineSDK(...)\` and any
// \`WaybackMachineSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+WaybackMachineSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/WaybackMachineSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
