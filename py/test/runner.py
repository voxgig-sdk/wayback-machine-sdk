# WaybackMachine SDK test runner

from __future__ import annotations
import os
import json

from waybackmachine_sdk.utility.voxgig_struct import voxgig_struct as vs


class WaybackMachineTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            WaybackMachineTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = WaybackMachineTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = WaybackMachineTestRunner.getenv("WAYBACK_MACHINE_TEST_LIVE")
        override = WaybackMachineTestRunner.getenv("WAYBACK_MACHINE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = WaybackMachineTestRunner.getenv(key)
        ...
        explain = WaybackMachineTestRunner.getenv("WAYBACK_MACHINE_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if WaybackMachineTestRunner._test_control is not None:
            return WaybackMachineTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                WaybackMachineTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            WaybackMachineTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = WaybackMachineTestRunner.load_test_control()
...
def load_env_local():
    WaybackMachineTestRunner.load_env_local()


def env_override(m):
    return WaybackMachineTestRunner.env_override(m)


def entity_data(v):
    return WaybackMachineTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return WaybackMachineTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return WaybackMachineTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return WaybackMachineTestRunner.load_test_control()


def live_client_options():
    return WaybackMachineTestRunner.live_client_options()


def live_delay_ms():
    return WaybackMachineTestRunner.live_delay_ms()
