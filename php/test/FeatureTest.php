...
// WaybackMachine SDK feature test
//
...
class FtCtrl extends WaybackMachineControl
{
...
class FtClient
{
    ...
    private WaybackMachineUtility $utility;

    public function __construct(WaybackMachineUtility $utility, array $options)
    {
    ...
    public function get_utility(): WaybackMachineUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (WaybackMachineContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public WaybackMachineContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (WaybackMachineContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = WaybackMachineConfig::make_feature($name);
    ...
    private static function buildUrl(WaybackMachineSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof WaybackMachineError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new WaybackMachineResponse($fetched);
        }
    ...
    private function fail(WaybackMachineContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (WaybackMachineContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
