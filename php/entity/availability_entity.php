...
class AvailabilityEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new AvailabilityEntity($this->_client, $opts);
    }

    /**
     * @param Availability|array $args Availability data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = WaybackMachineHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Availability|array The current Availability data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Availability fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = WaybackMachineHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Availability fields).
     */
     ...
     *
     * @param AvailabilityLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed AvailabilityLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Availability|array The loaded Availability as an assoc-array at the
     *   SDK boundary; throws WaybackMachineError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = WaybackMachineHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
